...
tx = pd.DataFrame({'tx_lat':[47.4791692], 'tx_lon' : [19.0561941]})
rx = pd.read_csv('Data2\\Logs\\1_2QPSK_meas.csv', sep=';').astype(float)
#rssi = rx['rx_power']
...
def CUSTOM_create_df_to_matlab():
    ...
    df.to_csv('Data2\\Logs\\custom_clustered_data.csv', index=False, sep=';')

CUSTOM_create_df_to_matlab()

from_matlab = pd.read_csv('Data2\\Logs\\propModellData.csv')
...
def stat():
    for cluster_id, cluster_data in clusters.items():
        ...
        Corrected_df.loc[:, 'Corrected RT Matlab'] = Corrected_df['RT Matlab']*avgrate
        Corrected_df = Corrected_df[np.abs(stats.zscore(Corrected_df["Corrected RT Matlab"])) < 2]
        ...
        model1 = LinearRegression()
        model1.fit(X_poly, y)
        plt.plot(X, model1.predict(X_poly), color='blue', label='Real Measurement', linestyle = 'solid')
        ...
        model2 = LinearRegression()
        model2.fit(X_poly, y)
        plt.plot(X, model2.predict(X_poly), color='gold', label='Corrected RT Matlab', linestyle = 'solid')
        ...
        model3 = LinearRegression()
        model3.fit(X_poly, y)
        plt.plot(X, model3.predict(X_poly), color='green', label='RT Matlab', linestyle = 'solid')
        ...
        correlation_corrected = np.corrcoef(Corrected_df['RX Power'], Corrected_df['Corrected RT Matlab'])[0, 1]
        #corr.append(f"The correlation in cluster {cluster_id} is {correlation:.2f}")
        corr.append((cluster_id, (correlation, correlation_corrected)))
    ...
    for cluster_id, correlation in corr:
        print(f"The correlation in cluster {cluster_id} is {correlation:.3f}")
        print(f"The correlation of corrected data in cluster {cluster_id} is {correlation_corrected:.3f}")
